        ...
        public static IEnumerable<Type> GetAllDerivedTypes(Type[] baseTypes, string[] assemblyNames)
        {
            ...
            return assemblies.Where(asm => !asm.IsDynamic && assemblyNamesHashset.Contains(asm.GetName().Name))
                ...
                .Where(type =>
                    ...
                    baseTypes.Any(baseType => baseType.IsAssignableFrom(type)));
        }

        public static IEnumerable<Type> GetTypesInNamespace(Type[] baseTypes, string[] assemblyNames, string nameSpace)
        {
            return GetAllDerivedTypes(baseTypes, assemblyNames)
                .Where(t => t.Namespace != null && t.Namespace.StartsWith(nameSpace));
        }

        public static void CopyTo(this object from, object to)
        {
            var fromFieldInfos = from.GetType()
                .GetFields(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
            var toFieldInfos =
                to.GetType().GetFields(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
            foreach (var fromField in fromFieldInfos)
            {
                var toField = toFieldInfos.FirstOrDefault(toField =>
                    toField.Name == fromField.Name && toField.FieldType.IsAssignableFrom(fromField.FieldType));
                if (toField != null)
                {
