...
namespace BestHTTP
{
    ...
    public sealed class HTTPProxy : Proxy
    {
        ...
        internal override bool SetupRequest(HTTPRequest request)
        {
            ...
            var authHeader = DigestStore.FindBest(request.Response.GetHeaderValues("proxy-authenticate"));
                    ...
                    switch (request.Proxy.Credentials.Type)
                    {
                        case AuthenticationTypes.Basic:
                            ...
                            request.SetHeader("Proxy-Authorization",
                                string.Concat("Basic ", Convert.ToBase64String(Encoding.UTF8.GetBytes(request.Proxy.Credentials.UserName + ":" + request.Proxy.Credentials.Password))));
                            return true;
                        ...
                        case AuthenticationTypes.Digest:
                            ...
                            if (digest != null)
                            {
                                var authentication = digest.GenerateResponseHeader(request, request.Proxy.Credentials, true);
