...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
    ...
    internal abstract class Ssl3Utilities
    {
        ...
        {
            var prf       = handshakeHash.ForkPrfHash();
            var sslSender = isServer ? SSL_SERVER : SSL_CLIENT;
        ...
        {
            var masterSecret  = context.SecurityParameters.MasterSecret;
            var master_secret = context.Crypto.AdoptSecret(masterSecret).Extract();
        ...
        internal static byte[] ReadEncryptedPms(Stream input) { return Streams.ReadAll(input); }

        /// <exception cref="IOException" />
        internal static void WriteEncryptedPms(byte[] encryptedPms, Stream output) { output.Write(encryptedPms, 0, encryptedPms.Length); }
    }
}
