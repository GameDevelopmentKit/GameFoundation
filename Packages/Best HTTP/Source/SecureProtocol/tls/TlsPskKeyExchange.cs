...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
        ...
        : AbstractTlsKeyExchange
    {
        ...
        public override void GenerateClientKeyExchange(Stream output)
        {
            ...
            var psk_identity = this.m_pskIdentity.GetPskIdentity();
            ...
            this.m_context.SecurityParameters.m_pskIdentity = Arrays.Clone(psk_identity);
        ...
        {
            var psk_identity = TlsUtilities.ReadOpaque16(input);

            this.m_psk = this.m_pskIdentityManager.GetPsk(psk_identity);
            if (this.m_psk == null)
                throw new TlsFatalAlert(AlertDescription.unknown_psk_identity);

            this.m_context.SecurityParameters.m_pskIdentity = psk_identity;
        ...
        {
            var other_secret = this.GenerateOtherSecret(this.m_psk.Length);

            var buf = new MemoryStream(4 + other_secret.Length + this.m_psk.Length);
            TlsUtilities.WriteOpaque16(other_secret, buf);
            TlsUtilities.WriteOpaque16(this.m_psk, buf);

            Array.Clear(this.m_psk, 0, this.m_psk.Length);
            this.m_psk = null;

            return this.m_context.Crypto.CreateSecret(buf.ToArray());
        }
        ...
        protected virtual byte[] GenerateOtherSecret(int pskLength)
        {
            ...
            if (this.m_keyExchange == KeyExchangeAlgorithm.DHE_PSK ||
                ...
                if (this.m_agreement != null)
                    return this.m_agreement.CalculateSecret().Extract();

            if (this.m_keyExchange == KeyExchangeAlgorithm.RSA_PSK)
                if (this.m_preMasterSecret != null)
