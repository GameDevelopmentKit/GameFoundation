...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls.Crypto.Impl.BC
{
        ...
        : TlsDHDomain
    {
        ...
        private static int GetValueLength(DHParameters dh) { return (dh.P.BitLength + 7) / 8; }
        ...
        {
            var basicAgreement = new DHBasicAgreement();
            basicAgreement.Init(privateKey);
            var agreementValue = basicAgreement.CalculateAgreement(publicKey);
            var secret         = EncodeValue(privateKey.Parameters, padded, agreementValue);
            return crypto.AdoptLocalSecret(secret);
        }

        public static DHParameters GetParameters(TlsDHConfig dhConfig)
        {
            var dhGroup = TlsDHUtilities.GetDHGroup(dhConfig);
            if (dhGroup == null)
        ...
        public virtual TlsAgreement CreateDH() { return new BcTlsDH(this); }

        /// <exception cref="IOException" />
        public virtual BigInteger DecodeParameter(byte[] encoding)
        {
            if (this.dhConfig.IsPadded && GetValueLength(this.dhParameters) != encoding.Length)
        ...
        public virtual DHPublicKeyParameters DecodePublicKey(byte[] encoding)
        {
            ...
            try
            {
                var y = this.DecodeParameter(encoding);
        ...
        /// <exception cref="IOException" />
        public virtual byte[] EncodeParameter(BigInteger x) { return EncodeValue(this.dhParameters, this.dhConfig.IsPadded, x); }

        /// <exception cref="IOException" />
        public virtual byte[] EncodePublicKey(DHPublicKeyParameters publicKey) { return EncodeValue(this.dhParameters, true, publicKey.Y); }

        public virtual AsymmetricCipherKeyPair GenerateKeyPair()
        {
            var keyPairGenerator = new DHBasicKeyPairGenerator();
            keyPairGenerator.Init(new DHKeyGenerationParameters(this.crypto.SecureRandom, this.dhParameters));
