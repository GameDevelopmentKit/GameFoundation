...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls.Crypto.Impl.BC
{
        ...
        : DefaultTlsCredentialedSigner
    {
            ...
            Certificate certificate, SignatureAndHashAlgorithm signatureAndHashAlgorithm)
        {
            ...
            {
                var privKeyRsa = (RsaKeyParameters)privateKey;

                if (signatureAndHashAlgorithm != null)
                {
                    var signatureScheme = SignatureScheme.From(signatureAndHashAlgorithm);
                    if (SignatureScheme.IsRsaPss(signatureScheme)) return new BcTlsRsaPssSigner(crypto, privKeyRsa, signatureScheme);
                }

                var pubKeyRsa = GetEndEntity(crypto, certificate).GetPubKeyRsa();
            ...
            {
                var privKeyEC = (ECPrivateKeyParameters)privateKey;

                if (signatureAndHashAlgorithm != null)
                {
                    var signatureScheme = SignatureScheme.From(signatureAndHashAlgorithm);
                    if (SignatureScheme.IsECDsa(signatureScheme)) return new BcTlsECDsa13Signer(crypto, privKeyEC, signatureScheme);
                }
