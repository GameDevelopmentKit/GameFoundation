...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
    ...
    public sealed class CertificateUrl
    {
        ...
        public void Encode(Stream output)
        {
            ...
            var buf = new ListBuffer16();
            foreach (UrlAndHash urlAndHash in this.UrlAndHashList) urlAndHash.Encode(buf);
            buf.EncodeTo(output);
        }
        ...
        public static CertificateUrl Parse(TlsContext context, Stream input)
        {
            ...
            var totalLength = TlsUtilities.ReadUint16(input);
            if (totalLength < 1)
                throw new TlsFatalAlert(AlertDescription.decode_error);

            var urlAndHashListData = TlsUtilities.ReadFully(totalLength, input);

            var buf = new MemoryStream(urlAndHashListData, false);

            var url_and_hash_list = Platform.CreateArrayList();
            while (buf.Position < buf.Length)
            {
                var url_and_hash = UrlAndHash.Parse(context, buf);
