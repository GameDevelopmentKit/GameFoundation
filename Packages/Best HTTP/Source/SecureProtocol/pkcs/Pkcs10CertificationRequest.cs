...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Pkcs
{
    ...
    /// </code>
    /// see
    /// <a href="http://www.rsasecurity.com/rsalabs/node.asp?id=2132" />
        ...
        : CertificationRequest
    {
        ...
        public AsymmetricKeyParameter GetPublicKey() { return PublicKeyFactory.CreateKey(this.reqInfo.SubjectPublicKeyInfo); }

        /// <summary>
        ///     Verify Pkcs10 Cert Request is valid.
        /// </summary>
        /// <returns>true = valid.</returns>
        public bool Verify() { return this.Verify(this.GetPublicKey()); }
        ...
        {
            var asn1Params = sigAlgId.Parameters;

            if (asn1Params != null && !(asn1Params is Asn1Null))
                if (sigAlgId.Algorithm.Equals(PkcsObjectIdentifiers.IdRsassaPss))
                {
                    var rsaParams = RsassaPssParameters.GetInstance(asn1Params);
        ...
        public X509Extensions GetRequestedExtensions()
        {
            if (this.reqInfo.Attributes != null)
                foreach (Asn1Encodable item in this.reqInfo.Attributes)
                {
                    ...
                    if (attributePkcs.AttrType.Equals(PkcsObjectIdentifiers.Pkcs9AtExtensionRequest))
                    {
                        ...
                        var extensionSequence = Asn1Sequence.GetInstance(attributePkcs.AttrValues[0]);


                        foreach (Asn1Encodable seqItem in extensionSequence)
                        {
                            var itemSeq = Asn1Sequence.GetInstance(seqItem);
                            if (itemSeq.Count == 2)
