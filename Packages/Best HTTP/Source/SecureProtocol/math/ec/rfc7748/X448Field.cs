...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Math.EC.Rfc7748
{
    ...
    public abstract class X448Field
    {
        ...
        {
            uint d                           = 0;
            for (var i = 0; i < Size; ++i) d |= x[i] ^ y[i];
            d |= d >> 16;
            d &= 0xFFFF;
            return ((int)d - 1) >> 31;
        }

        public static bool AreEqualVar(uint[] x, uint[] y) { return 0 != AreEqual(x, y); }
        ...
        public static uint[] CreateTable(int n) { return new uint[Size * n]; }
