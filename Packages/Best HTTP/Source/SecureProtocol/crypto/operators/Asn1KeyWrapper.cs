...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Crypto.Operators
{
        ...
        : IKeyWrapper
    {
        ...
        public Asn1KeyWrapper(DerObjectIdentifier algorithm, Asn1Encodable parameters, ICipherParameters key)
        {
            ...
            {
                var             oaepParams = RsaesOaepParameters.GetInstance(parameters);
                WrapperProvider provider;
                if (oaepParams.MaskGenAlgorithm.Algorithm.Equals(PkcsObjectIdentifiers.IdMgf1))
                {
                    var digAlg = AlgorithmIdentifier.GetInstance(oaepParams.MaskGenAlgorithm.Parameters);
                ...
                this.wrapper = (IKeyWrapper)provider.CreateWrapper(true, key);
            }
            else if (algorithm.Equals(PkcsObjectIdentifiers.RsaEncryption))
            {
                this.wrapper = new RsaPkcs1Wrapper(true, key);
            }
            else
            {
    ...
    {
        private          string        algorithm;
        private readonly IKeyUnwrapper wrapper;
        ...
        public Asn1KeyUnwrapper(DerObjectIdentifier algorithm, Asn1Encodable parameters, ICipherParameters key)
        {
            ...
            if (algorithm.Equals(PkcsObjectIdentifiers.IdRsaesOaep))
            {
                ...
                this.wrapper = (IKeyUnwrapper)provider.CreateWrapper(false, key);
            }
            else if (algorithm.Equals(PkcsObjectIdentifiers.RsaEncryption))
            {
                var             oaepParams = RsaesOaepParameters.GetInstance(parameters);
                ...
                this.wrapper = new RsaPkcs1Wrapper(false, key);
            }
            else
            {
                throw new ArgumentException("unknown algorithm: " + algorithm.Id);
        ...
        public IBlockResult Unwrap(byte[] keyData, int offSet, int length) { return this.wrapper.Unwrap(keyData, offSet, length); }
    }
    ...
    internal class RsaPkcs1Wrapper : IKeyWrapper, IKeyUnwrapper
    {
        ...
        public IBlockResult Unwrap(byte[] cipherText, int offset, int length) { return new SimpleBlockResult(this.engine.ProcessBlock(cipherText, offset, length)); }

        public IBlockResult Wrap(byte[] keyData) { return new SimpleBlockResult(this.engine.ProcessBlock(keyData, 0, keyData.Length)); }
    }

    internal class RsaPkcs1WrapperProvider
        : WrapperProvider
    {
        object WrapperProvider.CreateWrapper(bool forWrapping, ICipherParameters parameters) { return new RsaPkcs1Wrapper(forWrapping, parameters); }
    }
        ...
        : WrapperProvider
    {
        ...
        {
            return new RsaOaepWrapper(forWrapping, parameters, this.digestOid, this.mgfOid);
        }
    }
}
