...
namespace BestHTTP.SignalRCore.Encoders
{
    ...
    public sealed class MessagePackProtocol : IProtocol
    {
        ...
        public MessagePackProtocol(MessagePackProtocolSerializationOptions options)
        {
            ...
            Json.DefaultSerializers.Clear();
            Json.DefaultSerializers.AddRange(new TypeSerializer[]
            {
        ...
        public void ParseMessages(BufferSegment segment, ref List<Message> messages)
        {
            ...
            while (offset < segment.Count)
            {
                ...
                using (var stream = new MemoryStream(segment.Data, offset, length))
                {
        ...
        private object[] ReadArguments(MsgPackReader reader, string target)
        {
            ...
            else
            {
                ...
                if (subscription.callbacks[0].ParamTypes != null)
                {
                    ...
                    for (var i = 0; i < subscription.callbacks[0].ParamTypes.Length; ++i)
                        ...
                        case 12:
                            {
                                ...
                                value = UnixEpoch.AddSeconds(longValue).AddTicks(nanoseconds / NanosecondsPerTick);
                                return true;
                            }
                        default:
