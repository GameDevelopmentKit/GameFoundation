...
public partial class TranslucentImage : Image, IMeshModifier
{
    ...
    protected override void Start()
    {
        ...
        if (this.source && this.material) this.material.SetTexture(_blurTexPropId, this.source.BlurredScreen);
    ...
    {
        if (this.IsInPrefabMode()) return;
        if (this.sourceAcquiredOnStart) return;

        this.source                = this.source ? this.source : FindObjectOfType<TranslucentImageSource>();
    ...
    private bool Validate()
    {
        ...
        if (!source)
        {
            if (Application.isPlaying && !this.IsInPrefabMode())
        ...
        if (!this.source.BlurredScreen)
            return false;

        // Have to use string comparison as Addressable break object comparision :(
        if (this.material.shader.name != "UI/TranslucentImage")
        {
