...
namespace vietlabs.fr2
{
    ...
    public class FR2_Unity
    {
        ...
        public static bool StringStartsWith(string source, params string[] prefixes)
        {
            ...
            for (var i = 0; i < prefixes.Length; i++)
                if (source.StartsWith(prefixes[i]))
                    return true;
 ...
 typeof(Selection).GetProperty("assetGUIDs", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
        ...
        internal static int Epoch(DateTime time) { return (int)(time.ToUniversalTime() - new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc)).TotalSeconds; }
        ...
        internal static void RepaintFR2Windows() { RepaintAllEditor("vietlabs.fr2.FR2_Window"); }
        ...
        public static IEnumerable<GameObject> getAllObjsInCurScene()
        {
            ...
            for (var j = 0; j < SceneManager.sceneCount; j++)
            {
                ...
                foreach (var item in GetGameObjectsInScene(scene)) yield return item;
            }
            ...
            if (EditorApplication.isPlaying)
            {
                ...
                try
                {
                    ...
                    foreach (var item in GetGameObjectsInScene(dontDestroyOnLoad)) yield return item;
                }
                finally
                {
        ...
        private static IEnumerable<GameObject> GetGameObjectsInScene(Scene scene)
        {
            ...
            for (var i = 0; i < rootObjects.Count; ++i)
            {
                ...
                foreach (var item in getAllChild(gameObject)) yield return item;
        ...
        public static IEnumerable<GameObject> getAllChild(GameObject target, bool returnMe = false)
        {
            ...
            if (target.transform.childCount > 0)
                for (var i = 0; i < target.transform.childCount; i++)
                {
                    ...
                    foreach (var item in getAllChild(target.transform.GetChild(i).gameObject)) yield return item;
                }
        }
        ...
        public class TerrainTextureData
        {
            ...
            public TerrainTextureData(params Texture2D[] param)
            {
                ...
                for (var i = 0; i < count; i++) this.textures[i] = param[i];
            }
        }
        ...
        public static void BackupAndDeleteAssets(FR2_Ref[] assets)
        {
            ...
            try
            {
                for (var i = 0; i < result.Count; i++) AssetDatabase.DeleteAsset(result[i]);
            }
            finally
            {
