...
namespace vietlabs.fr2
{
    ...
    public class FR2_Export
    {
        ...
        private static void Ignore()
        {
            ...
            {
                var path = AssetDatabase.GetAssetPath(actives[i]);
        ...
        public static void MergeDuplicate(string guid_file)
        {
            ...
            var guids_files = FR2_Unity._Selection_AssetGUIDs;

            var realKey = "";
            foreach (var item in guids_files)
                if (item.StartsWith(guid_file, StringComparison.Ordinal))
                    realKey = item;
            ...
            foreach (var item in cacheSelection) cacheSelection.Add(item);
            ...
            foreach (var item in listReplace) item.Value.processIndex = item.Value.datas.Count - 1;
        ...
        {
            var notComplete = listReplace.Where(x => x.Value.processIndex >= 0).Count() > 0;
            ...
            for (var i = lstThreads.Count - 1; i >= 0; i--)
        ...
        private static void SelectDependencies(bool includeMe)
        {
            ...
            for (var i = 0; i < list.Count; i++) AddToDict(dict, FR2_Asset.FindUsage(list[i]).ToArray());
        ...
        private static void SelectUsed(bool includeMe)
        {
            ...
            for (var i = 0; i < list.Count; i++) AddToDict(dict, list[i].UsedByMap.Values.ToArray());
