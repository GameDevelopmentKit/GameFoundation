...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
    ...
    public abstract class TlsDHUtilities
    {
        ...
        public static int GetNamedGroupForDHParameters(BigInteger p, BigInteger g)
        {
            ...
            {
                var namedGroup = namedGroups[i];
                var dhGroup    = GetNamedDHGroup(namedGroup);
        ...
        public static DHGroup GetStandardGroupForDHParameters(BigInteger p, BigInteger g)
        {
            ...
            for (var i = 0; i < standardGroups.Length; ++i)
            {
                var dhGroup = standardGroups[i];
                if (dhGroup != null && dhGroup.P.Equals(p) && dhGroup.G.Equals(g))
                    return dhGroup;
            }
        ...
        public static TlsDHConfig ReceiveDHConfig(TlsContext context, TlsDHGroupVerifier dhGroupVerifier,
            Stream input)
        {
            var p = ReadDHParameter(input);
            var g = ReadDHParameter(input);

            var namedGroup = GetNamedGroupForDHParameters(p, g);
            if (namedGroup < 0)
            {
                var dhGroup                  = GetStandardGroupForDHParameters(p, g);
                if (null == dhGroup) dhGroup = new DHGroup(p, null, g, 0);
            ...
            var clientSupportedGroups = context.SecurityParameters.ClientSupportedGroups;
        ...
        /// <exception cref="IOException" />
        public static BigInteger ReadDHParameter(Stream input) { return new BigInteger(1, TlsUtilities.ReadOpaque16(input, 1)); }

        /// <exception cref="IOException" />
        public static void WriteDHConfig(TlsDHConfig dhConfig, Stream output)
        {
            var group = GetDHGroup(dhConfig);
