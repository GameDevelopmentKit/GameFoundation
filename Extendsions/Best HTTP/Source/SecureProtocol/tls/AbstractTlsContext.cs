...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
        ...
        : TlsContext
    {
        ...
        internal void HandshakeBeginning(TlsPeer peer)
        {
            lock (this)
            {
                if (null != this.m_securityParameters)
                    throw new TlsFatalAlert(AlertDescription.internal_error, "Handshake already started");

                this.m_securityParameters          = new SecurityParameters();
        ...
        internal bool IsHandshaking
        {
            get
            {
                lock (this)
                {
                    return !this.m_connected && null != this.m_securityParameters;
                }
            }
        }
        ...
        internal void SetClientSupportedVersions(ProtocolVersion[] clientSupportedVersions) { this.m_clientSupportedVersions = clientSupportedVersions; }

        public virtual ProtocolVersion ClientVersion => this.m_clientVersion;

        internal void SetClientVersion(ProtocolVersion clientVersion) { this.m_clientVersion = clientVersion; }

        public virtual ProtocolVersion RsaPreMasterSecretVersion => this.m_rsaPreMasterSecretVersion;

        internal void SetRsaPreMasterSecretVersion(ProtocolVersion rsaPreMasterSecretVersion) { this.m_rsaPreMasterSecretVersion = rsaPreMasterSecretVersion; }
        ...
        public virtual byte[] ExportChannelBinding(int channelBinding)
        {
            ...
            switch (channelBinding)
            {
                ...
                {
                    var tlsServerEndPoint = securityParameters.TlsServerEndPoint;
        ...
        public virtual byte[] ExportEarlyKeyingMaterial(string asciiLabel, byte[] context, int length)
        {
            ...
            var sp = this.SecurityParameters;

            return this.ExportKeyingMaterial13(this.CheckEarlyExportSecret(sp.EarlyExporterMasterSecret),
        ...
        public virtual byte[] ExportKeyingMaterial(string asciiLabel, byte[] context, int length)
        {
            ...
            if (TlsUtilities.IsTlsV13(sp.NegotiatedVersion))
                return this.ExportKeyingMaterial13(this.CheckExportSecret(sp.ExporterMasterSecret), sp.PrfCryptoHashAlgorithm,
                    asciiLabel, context, length);

            var seed = TlsUtilities.CalculateExporterSeed(sp, context);

            return TlsUtilities.Prf(sp, this.CheckExportSecret(sp.MasterSecret), asciiLabel, seed, length).Extract();
        }
            ...
            byte[] context, int length)
        {
            ...
            else if (!TlsUtilities.IsValidUint16(context.Length)) throw new ArgumentException("must have length less than 2^16 (or be null)", "context");
