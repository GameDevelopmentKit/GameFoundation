...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
    ...
    public abstract class SignatureScheme
    {
        ...
        public static int From(short hashAlgorithm, short signatureAlgorithm) { return ((hashAlgorithm & 0xFF) << 8) | (signatureAlgorithm & 0xFF); }
        ...
        public static int GetCryptoHashAlgorithm(int signatureScheme)
        {
            switch (signatureScheme)
            {
                ...
                default:
                {
                    var hashAlgorithm = GetHashAlgorithm(signatureScheme);
        ...
        {
            var hex = Platform.ToUpperInvariant(Convert.ToString(signatureScheme, 16));
            return GetName(signatureScheme) + "(0x" + hex + ")";
        }

        public static bool IsPrivate(int signatureScheme) { return signatureScheme >> 9 == 0xFE; }
