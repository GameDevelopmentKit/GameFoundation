...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls.Crypto.Impl.BC
{
        ...
        : TlsCredentialedDecryptor
    {
        ...
        {
            var secureRandom = this.m_crypto.SecureRandom;

            /*
             * RFC 5246 7.4.7.1.
             */
            var expectedVersion = cryptoParams.RsaPreMasterSecretVersion;

            // TODO Provide as configuration option?
            var versionNumberCheckDisabled = false;

            /*
             * Generate 48 random bytes we can use as a Pre-Master-Secret, if the
             * PKCS1 padding check should fail.
             */
            var fallback = new byte[48];
            secureRandom.NextBytes(fallback);

            var M = Arrays.Clone(fallback);
            try
            {
                var encoding = new Pkcs1Encoding(new RsaBlindedEngine(), fallback);
            ...
            else
            {
                 ...
                 */
                var mask = (expectedVersion.MajorVersion ^ (M[0] & 0xFF))
                           | (expectedVersion.MinorVersion ^ (M[1] & 0xFF));

                // 'mask' will be all 1s if the versions matched, or else all 0s.
                mask = (mask - 1) >> 31;

                for (var i = 0; i < 48; i++) M[i] = (byte)((M[i] & mask) | (fallback[i] & ~mask));
            }

            return this.m_crypto.CreateSecret(M);
        }
    }
}
