...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
        ...
        : AbstractTlsPeer, TlsClient
    {
        ...
        {
            var   namedGroupRoles = TlsUtilities.GetNamedGroupRoles(this.GetCipherSuites());
            IList sigAlgs         = this.m_supportedSignatureAlgorithms, sigAlgsCert = this.m_supportedSignatureAlgorithmsCert;
        ...
        /// <exception cref="IOException" />
        public virtual TlsPskIdentity GetPskIdentity() { return null; }

        /// <exception cref="IOException" />
        public virtual TlsSrpIdentity GetSrpIdentity() { return null; }

        public virtual TlsDHGroupVerifier GetDHGroupVerifier() { return new DefaultTlsDHGroupVerifier(); }

        public virtual TlsSrpConfigVerifier GetSrpConfigVerifier() { return new DefaultTlsSrpConfigVerifier(); }

        protected virtual IList GetCertificateAuthorities() { return null; }

        protected virtual IList GetProtocolNames() { return null; }

        protected virtual CertificateStatusRequest GetCertificateStatusRequest() { return new CertificateStatusRequest(CertificateStatusType.ocsp, new OcspStatusRequest(null, null)); }

        /// <returns>an <see cref="IList" /> of <see cref="CertificateStatusRequestItemV2" /> (or null).</returns>
        protected virtual IList GetMultiCertStatusRequest() { return null; }

        protected virtual IList GetSniServerNames() { return null; }

        /// <summary>
        ///     The default <see cref="GetClientExtensions" /> implementation calls this to determine which named
        ///     groups to include in the supported_groups extension for the ClientHello.
        /// </summary>
        /// <param name="namedGroupRoles">
        ///     The <see cref="NamedGroupRole">named group roles</see> for which there should
        ///     be at least one supported group. By default this is inferred from the offered cipher suites and signature
        ///     algorithms.
        /// </param>
        /// <returns>
        ///     an <see cref="IList" /> of <see cref="int" />. See <see cref="NamedGroup" /> for group constants.
        /// </returns>
        protected virtual IList GetSupportedGroups(IList namedGroupRoles)
        {
            var crypto          = this.Crypto;
            var supportedGroups = Platform.CreateArrayList();

            if (namedGroupRoles.Contains(NamedGroupRole.ecdh))
                TlsUtilities.AddIfSupported(supportedGroups, crypto,
                    new[] { NamedGroup.x25519, NamedGroup.x448 });

            if (namedGroupRoles.Contains(NamedGroupRole.ecdh) ||
                namedGroupRoles.Contains(NamedGroupRole.ecdsa))
                TlsUtilities.AddIfSupported(supportedGroups, crypto,
                    new[] { NamedGroup.secp256r1, NamedGroup.secp384r1 });

            if (namedGroupRoles.Contains(NamedGroupRole.dh))
                TlsUtilities.AddIfSupported(supportedGroups, crypto,
                    new[] { NamedGroup.ffdhe2048, NamedGroup.ffdhe3072, NamedGroup.ffdhe4096 });

            return supportedGroups;
        }

        protected virtual IList GetSupportedSignatureAlgorithms() { return TlsUtilities.GetDefaultSupportedSignatureAlgorithms(this.m_context); }

        protected virtual IList GetSupportedSignatureAlgorithmsCert() { return null; }

        protected virtual IList GetTrustedCAIndication() { return null; }
        ...
        public virtual TlsSession GetSessionToResume() { return null; }

        public virtual IList GetExternalPsks() { return null; }
        ...
        {
            var clientExtensions = Platform.CreateHashtable();

            var offeringTlsV13Plus = false;
            var offeringPreTlsV13  = false;
            {
                var supportedVersions = this.GetProtocolVersions();
            ...
            var protocolNames = this.GetProtocolNames();
            if (protocolNames != null) TlsExtensionsUtilities.AddAlpnExtensionClient(clientExtensions, protocolNames);

            var sniServerNames = this.GetSniServerNames();
            if (sniServerNames != null) TlsExtensionsUtilities.AddServerNameExtensionClient(clientExtensions, sniServerNames);

            var statusRequest = this.GetCertificateStatusRequest();
            if (statusRequest != null) TlsExtensionsUtilities.AddStatusRequestExtension(clientExtensions, statusRequest);

            if (offeringTlsV13Plus)
            {
                var certificateAuthorities = this.GetCertificateAuthorities();
                if (certificateAuthorities != null) TlsExtensionsUtilities.AddCertificateAuthoritiesExtension(clientExtensions, certificateAuthorities);
            }
            ...
            if (offeringPreTlsV13)
            {
                ...
                var statusRequestV2 = this.GetMultiCertStatusRequest();
                if (statusRequestV2 != null) TlsExtensionsUtilities.AddStatusRequestV2Extension(clientExtensions, statusRequestV2);

                var trustedCAKeys = this.GetTrustedCAIndication();
                if (trustedCAKeys != null) TlsExtensionsUtilities.AddTrustedCAKeysExtensionClient(clientExtensions, trustedCAKeys);
            }
            ...
            {
                var supportedSigAlgs = this.GetSupportedSignatureAlgorithms();
                ...
                var supportedSigAlgsCert = this.GetSupportedSignatureAlgorithmsCert();
            ...
            var namedGroupRoles = this.GetNamedGroupRoles();

            var supportedGroups = this.GetSupportedGroups(namedGroupRoles);
        ...
        public virtual IList GetEarlyKeyShareGroups()
        {
            ...
            if (null == this.m_supportedGroups || this.m_supportedGroups.Count < 1)
                return null;

            if (this.m_supportedGroups.Contains(NamedGroup.x25519))
                return TlsUtilities.VectorOfOne(NamedGroup.x25519);

            if (this.m_supportedGroups.Contains(NamedGroup.secp256r1))
                return TlsUtilities.VectorOfOne(NamedGroup.secp256r1);

            return TlsUtilities.VectorOfOne(this.m_supportedGroups[0]);
        }
        ...
        public virtual void ProcessServerExtensions(IDictionary serverExtensions)
        {
            ...
            else
            {
                 ...
                 */
                this.CheckForUnexpectedServerExtension(serverExtensions, ExtensionType.signature_algorithms);
                this.CheckForUnexpectedServerExtension(serverExtensions, ExtensionType.signature_algorithms_cert);

                this.CheckForUnexpectedServerExtension(serverExtensions, ExtensionType.supported_groups);

                var selectedCipherSuite = securityParameters.CipherSuite;
                ...
                if (TlsEccUtilities.IsEccCipherSuite(selectedCipherSuite))
                    ...
                    this.CheckForUnexpectedServerExtension(serverExtensions, ExtensionType.ec_point_formats);

                /*
                 * RFC 7685 3. The server MUST NOT echo the extension.
                 */
                this.CheckForUnexpectedServerExtension(serverExtensions, ExtensionType.padding);
            }
        }
        ...
        /// <exception cref="IOException" />
        public virtual IList GetClientSupplementalData() { return null; }
