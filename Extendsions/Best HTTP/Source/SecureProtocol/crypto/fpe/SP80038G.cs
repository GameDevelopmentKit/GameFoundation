...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Crypto.Fpe
{
    ...
    internal class SP80038G
    {
        ...
        protected static readonly double LOG2       = Math.Log(2.0);
        protected static readonly double TWO_TO_96  = Math.Pow(2, 96);
        ...
        public static byte[] DecryptFF1(IBlockCipher cipher, int radix, byte[] tweak, byte[] buf, int off, int len)
        {
            ...
            var A = toShort(buf, off, u);
            var B = toShort(buf, off + u, v);

            var rv = decFF1(cipher, radix, tweak, n, u, v, A, B);
        ...
        {
            var t = T.Length;
            var b = (Ceil(Math.Log(radix) * v / LOG2) + 7) / 8;
            var d = (b + 3) / 4 * 4 + 4;

            var P = calculateP_FF1(radix, (byte)u, n, t);

            var bigRadix = BigInteger.ValueOf(radix);
            var modUV    = calculateModUV(bigRadix, u, v);
            ...
            {
                // i. - iv.
                var y = calculateY_FF1(cipher, bigRadix, T, b, d, i, P, A);

                // v.
                m = n - m;
                var modulus = modUV[i & 1];

                // vi.
                var c = num(bigRadix, B).Subtract(y).Mod(modulus);
        ...
        public static byte[] DecryptFF3(IBlockCipher cipher, int radix, byte[] tweak64, byte[] buf, int off, int len)
        {
            ...
            if (tweak64.Length != 8) throw new ArgumentException();
        ...
        public static byte[] DecryptFF3_1(IBlockCipher cipher, int radix, byte[] tweak56, byte[] buf, int off, int len)
        {
            ...
            if (tweak56.Length != 7) throw new ArgumentException("tweak should be 56 bits");

            var tweak64 = calculateTweak64_FF3_1(tweak56);
        ...
        private static ushort[] encFF1(IBlockCipher cipher, int radix, byte[] T, int n, int u, int v, ushort[] A, ushort[] B)
        {
            ...
            {
                // i. - iv.
                var y = calculateY_FF1(cipher, bigRadix, T, b, d, i, P, B);
                ...
                // vi.
                var c = num(bigRadix, A).Add(y).Mod(modulus);
        ...
        {
            var modUV = new BigInteger[2];
            modUV[0] = bigRadix.Pow(u);
            modUV[1] = modUV[0];
            if (v != u) modUV[1] = modUV[1].Multiply(bigRadix);
            return modUV;
        }

        protected static byte[] calculateP_FF1(int radix, byte uLow, int n, int t)
        {
            var P = new byte[BLOCK_SIZE];
            P[0] = 1;
            P[1] = 2;
            P[2] = 1;
        ...
        protected static BigInteger calculateY_FF1(IBlockCipher cipher, BigInteger bigRadix, byte[] T, int b, int d, int round, byte[] P, ushort[] AB)
        {
            ...
            // i.
            var numAB   = num(bigRadix, AB);
            var bytesAB = BigIntegers.AsUnsignedByteArray(numAB);

            var zeroes = -(t + b + 1) & 15;
            var Q      = new byte[t + zeroes + 1 + b];
            Array.Copy(T, 0, Q, 0, t);
            Q[t + zeroes] = (byte)round;
            Array.Copy(bytesAB, 0, Q, Q.Length - bytesAB.Length, bytesAB.Length);

            // ii.
            var R = prf(cipher, Arrays.Concatenate(P, Q));

            // iii.
            var sBlocks = R;
            if (d > BLOCK_SIZE)
            {
                var sBlocksLen = (d + BLOCK_SIZE - 1) / BLOCK_SIZE;
                sBlocks = new byte[sBlocksLen * BLOCK_SIZE];
                Array.Copy(R, 0, sBlocks, 0, BLOCK_SIZE);

                var uint32 = new byte[4];
                for (uint j = 1; j < sBlocksLen; ++j)
                {
                    var sOff = (int)(j * BLOCK_SIZE);
        ...
        {
            checkCipher(cipher);
            if (radix < 2 || radix > 1 << 16) throw new ArgumentException();
        ...
        {
            checkCipher(cipher);
            if (radix < 2 || radix > 1 << 8) throw new ArgumentException();
            checkData(isFF1, radix, buf, off, len);
        }

        protected static void checkCipher(IBlockCipher cipher)
        {
            if (BLOCK_SIZE != cipher.GetBlockSize()) throw new ArgumentException();
        }
        ...
        protected static void checkData(bool isFF1, int radix, ushort[] buf, int off, int len)
        {
            ...
            for (var i = 0; i < len; ++i)
            {
                var b = buf[off + i] & 0xFFFF;
                if (b >= radix) throw new ArgumentException("input data outside of radix");
            }
        }
        ...
        {
            if (len < 2 || Math.Pow(radix, len) < 1000000) throw new ArgumentException("input too short");
            if (!isFF1)
            {
                var maxLen = 2 * (int)Math.Floor(Math.Log(TWO_TO_96) / Math.Log(radix));
                if (len > maxLen) throw new ArgumentException("maximum input length is " + maxLen);
            }
        }
        ...
        protected static byte[] implDecryptFF3(IBlockCipher cipher, int radix, byte[] tweak64, byte[] buf, int off, int len)
        {
            ...
            var rv = decFF3_1(cipher, radix, T, n, v, u, A, B);
        ...
        {
            var bigRadix = BigInteger.ValueOf(radix);
            var modVU    = calculateModUV(bigRadix, v, u);
            ...
            for (var i = 7; i >= 0; --i)
            {
                // i.
                m = n - m;
                var modulus = modVU[1 - (i & 1)];
                var wOff    = 4 - (i & 1) * 4;

                // ii. - iv.
                var y = calculateY_FF3(cipher, bigRadix, T, wOff, (uint)i, A);
        ...
        protected static byte[] implEncryptFF3(IBlockCipher cipher, int radix, byte[] tweak64, byte[] buf, int off, int len)
        {
            ...
            var rv = encFF3_1(cipher, radix, T, n, v, u, A, B);
        ...
        private static ushort[] encFF3_1(IBlockCipher cipher, int radix, byte[] t, int n, int v, int u, ushort[] a, ushort[] b)
        {
            ...
            for (uint i = 0; i < 8; ++i)
            {
                ...
                m = n - m;
                var modulus = modVU[1 - (i & 1)];
                var wOff    = 4 - (int)((i & 1) * 4);

                // ii. - iv.
                var y = calculateY_FF3(cipher, bigRadix, t, wOff, i, b);
        ...
        protected static BigInteger num(BigInteger R, ushort[] x)
        {
            ...
            for (var i = 0; i < x.Length; ++i) result = result.Multiply(R).Add(BigInteger.ValueOf(x[i] & 0xFFFF));
            return result;
        }

        protected static byte[] prf(IBlockCipher c, byte[] x)
        {
            if (x.Length % BLOCK_SIZE != 0) throw new ArgumentException();

            var m = x.Length / BLOCK_SIZE;
            var y = new byte[BLOCK_SIZE];
        ...
        {
            if (x.SignValue < 0) throw new ArgumentException();
            for (var i = 1; i <= m; ++i)
            {
                var qr = x.DivideAndRemainder(R);
        ...
        {
            for (var i = 0; i < len; ++i) y[yOff + i] ^= x[xOff + i];
        }

        private static byte[] toByte(ushort[] buf)
        {
            var s = new byte[buf.Length];

            for (var i = 0; i != s.Length; i++) s[i] = (byte)buf[i];
        ...
        {
            var s = new ushort[len];

            for (var i = 0; i != s.Length; i++) s[i] = (ushort)(buf[off + i] & 0xFF);
