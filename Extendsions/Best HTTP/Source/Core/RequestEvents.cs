namespace BestHTTP.Core
{
    ...
    using Timer = BestHTTP.Extensions.Timer;
        ...
        public override string ToString()
        {
            switch (this.Event)
            {
                ...
                case RequestEvents.DownloadProgress:
                    return string.Format("[RequestEventInfo SourceRequest: {0}, Event: DownloadProgress, Progress: {1}, ProgressLength: {2}]", this.SourceRequest.CurrentUri, this.Progress,
                        this.ProgressLength);
                case RequestEvents.UploadProgress:
                    return string.Format("[RequestEventInfo SourceRequest: {0}, Event: UploadProgress, Progress: {1}, ProgressLength: {2}]", this.SourceRequest.CurrentUri, this.Progress,
                        this.ProgressLength);
                ...
                case RequestEvents.TimingData:
                    if (this.Duration == TimeSpan.Zero)
                        ...
                        return string.Format("[RequestEventInfo SourceRequest: {0}, Event: TimingData, Name: {1}, Time: {2}, Duration: {3}]", this.SourceRequest.CurrentUri, this.Name, this.Time,
                            this.Duration);
                default:
                    ...
                    case RequestEvents.StreamingData:
                        {
                            ...
                            if (response != null)
                                Interlocked.Decrement(ref response.UnprocessedFragments);
            ...
            switch (@event.State)
            {
                ...
                case HTTPRequestStates.Finished:
                    ...
                    try
                    {
                        ...
                        if (tryLoad && HTTPCacheService.IsCachedEntityExpiresInTheFuture(source))
                        {
                            ...
                            ThreadedRunner.RunShortLiving(req =>
                            {
                                ...
                                if (ConnectionHelper.TryLoadAllFromCache("RequestEventHelper", req, req.Context))
                                {
