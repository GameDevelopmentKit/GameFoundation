...
namespace Com.ForbiddenByte.OSA.CustomAdapters.GridView.Specialized.GridWithCategories
{
    ...
    {
        private static T CreateItemModelForRowCompletion<T>(ICategoryModel parentCategory) where T : ICellModel, new()
        {
            return new()
            {
        ...
        private static T CreateItemModelInRowSeparatingCategories<T>(ICategoryModel parentCategory) where T : ICellModel, new()
        {
            return new()
            {
                ParentCategory = parentCategory,
            ...
            where TCell : ICellModel, new()
        {
            ...
            for (var i = 0; i < categories.Count; i++)
            {
                ...
                for (var j = 0; j < itemSlotsPerRow; j++)
                {
                ...
                for (var j = 0; j < cat.Count; j++) cells.Add((TCell)cat[j]);
                ...
                while (cells.Count % itemSlotsPerRow != 0) cells.Add(CreateItemModelForRowCompletion<TCell>(cat));
            }
        }
    }
}