...
namespace Com.ForbiddenByte.OSA.CustomAdapters.TableView.Tuple
{
    ...
    {
        [SerializeField] private RectTransform _EdgeDragger = null;
        public                   RectTransform EdgeDragger { get => this._EdgeDragger; set => this._EdgeDragger = value; }

        [SerializeField] private TableResizingMode _ResizingMode = TableResizingMode.NONE;
        public                   TableResizingMode ResizingMode { get => this._ResizingMode; set => this._ResizingMode = value; }

        public override void InitIfNeeded(IOSA iAdapter)
        {
            if (this.ItemPrefab == null || !this.ItemPrefab.gameObject.activeInHierarchy)
                throw new OSAException(
                    "A Tuple's value prefab should be non-null, and a scene object (i.e. not directly assigned from project view) active in hierarchy." + "This error may be caused by the Tuple prefab itself not being an object present in scene");

            base.InitIfNeeded(iAdapter);

            var tupleAdapter = iAdapter as ITupleAdapter;
            if (tupleAdapter == null) throw new OSAException(typeof(TupleParams).Name + ": No script implementing " + typeof(ITupleAdapter).Name + " found on TuplePrefab");
            var prefabRebuildNeeded = this.InitResizingNeeded(tupleAdapter);
            if (prefabRebuildNeeded)
                this.InitItemPrefab();
            else
                this.AssertValidWidthHeight(this.ItemPrefab);

            if (this.ContentPadding.left == -1 || this.ContentPadding.right == -1 || this.ContentPadding.top == -1 || this.ContentPadding.bottom == -1) throw new OSAException(typeof(TupleParams).Name + ": A Tuple's ContentPadding isn't allowed to have negative components in any direction. Set them to zero or positive values");
        ...
        private bool InitResizingNeeded(ITupleAdapter tupleAdapter)
        {
            ...
            if (this.ResizingMode == TableResizingMode.AUTO_FIT_TUPLE_CONTENT)
            {
                if (this.EdgeDragger && this.EdgeDragger.gameObject.activeSelf) this.EdgeDragger.gameObject.SetActive(false);

                if (tupleParams.ItemTransversalSize != -1f)
                {
                    Debug.Log(
                        typeof(TupleParams).Name + ": ItemTransversalSize needs to be -1, " + "because ResizingMode is " + this.ResizingMode + ". To avoid this message, manually set it in inspector"
                    );
            ...
            {
                if (this.EdgeDragger && !this.EdgeDragger.gameObject.activeSelf) this.EdgeDragger.gameObject.SetActive(true);

                if (tupleParams.ItemTransversalSize == -1f)
                {
                    Debug.Log(
                        typeof(TupleParams).Name + ": ItemTransversalSize set to -1 is not supported, " + "because ResizingMode is not " + TableResizingMode.AUTO_FIT_TUPLE_CONTENT + ". To avoid this message, manually set it to something else in inspector"
                    );
                ...
                if (csf && csf.enabled)
                {
                    Debug.Log(typeof(TupleParams).Name
                        + ": Found enabled ContentSizeFitter on tuple's value prefab, but ResizingMode is not "
                        + TableResizingMode.AUTO_FIT_TUPLE_CONTENT
                        + ". Disabling ContentSizeFitter...");
                    csf.enabled = false;
                }

                if (this.ResizingMode == TableResizingMode.NONE)
                {
                }
                else
                {
            ...
            if (this.ResizingMode == TableResizingMode.NONE)
            {
                if (layoutGroup && layoutGroup.enabled)
                {
                    ...
                    Debug.Log(typeof(TupleParams).Name + ": Found enabled LayoutGroup on tuple's value prefab, but ResizingMode is " + this.ResizingMode + ". Disabling LayoutGroup...");
                }
                ...
                foreach (RectTransform rt in valuePrefab)
                {
                    ...
                    if (le) le.enabled = false;
                }
            }
            else
            {
                if (layoutGroup && !layoutGroup.enabled) layoutGroup.enabled = true;
