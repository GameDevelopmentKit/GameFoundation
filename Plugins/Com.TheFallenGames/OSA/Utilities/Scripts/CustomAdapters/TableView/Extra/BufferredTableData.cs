...
namespace Com.ForbiddenByte.OSA.CustomAdapters.TableView.Extra
{
    ...
    public class BufferredTableData : TableData
    {
        ...
        public override int  Count                   => this._DataSource.Count;
        public override bool ColumnClearingSupported => false;

        private BufferredDataSource<ITuple> _DataSource;
        private TuplesChunkReader           _ChunkReader;
        ...
        {
            this._ChunkReader = tuplesChunkReader;
            this._DataSource  = new(tuplesCount, this.ReadTuplesChunk, chunkBufferSize, false /*items will be created directly*/);

            this.Init(columnsProvider, columnSortingSupported);
        }

        private void ReadTuplesChunk(ITuple[] into, int firstItemIndex, int countToRead)
        {
            this._ChunkReader(into, firstItemIndex, countToRead);
        }
        ...
        public bool TryGetCachedTuple(int index, out ITuple tuple)
        {
            return this._DataSource.TryGetCachedValue(index, out tuple);
        }

        /// <summary>See <see cref="BufferredDataSource{T}.GetValueUnchecked(int)"/></summary>
        public ITuple GetExistingTuple(int index)
        {
            return this._DataSource.GetValueUnchecked(index);
        }
