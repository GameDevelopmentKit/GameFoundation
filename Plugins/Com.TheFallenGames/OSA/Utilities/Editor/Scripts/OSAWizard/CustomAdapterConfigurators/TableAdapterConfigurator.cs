...
namespace Com.ForbiddenByte.OSA.Editor.OSAWizard.CustomAdapterConfigurators
{
    ...
    public class TableAdapterConfigurator : ICustomAdapterConfigurator
    {
        public void ConfigureNewAdapter(IOSA adapter)
        {
            ...
            p.ContentSpacing = 2;
            if (!p.IsHorizontal) p.ContentPadding.bottom = 320;
            ...
            if (csbTR) pTable.ColumnsScrollbar = csbTR.GetComponent<Scrollbar>();

            // Instantiate the tuple prefab & its value prefab
            pTable.TuplePrefab = this.InstantiateFromRes(paths.TUPLE_PREFAB, tuplePrefabsParent);
            var tupleAdapter = pTable.TuplePrefab.GetComponent(typeof(ITupleAdapter)) as ITupleAdapter;
            tupleAdapter.TupleParameters.ItemPrefab = this.InstantiateFromRes(paths.TUPLE_VALUE_PREFAB, tuplePrefabsParent);
            ...
            pTable.ColumnsTuplePrefab = this.InstantiateFromRes(paths.COLUMNS_TUPLE_PREFAB, columnsTuplePrefabsParent);
            var hTupleAdapter     = pTable.ColumnsTuplePrefab.GetComponent(typeof(ITupleAdapter)) as ITupleAdapter;
            var hTupleValuePrefab = hTupleAdapter.TupleParameters.ItemPrefab = this.InstantiateFromRes(paths.COLUMNS_TUPLE_VALUE_PREFAB, columnsTuplePrefabsParent);
            this.DisableLayoutComponentsInTupleValuePrefab(hTupleValuePrefab);
            ...
            Debug.Log("TableView instantiated. Resize mode was set to " + tupleAdapter.TupleParameters.ResizingMode + ". If you don't plan to have individual rows resizeable, " + "go over the tuple & column prefabs and their associated value prefabs and remove " + "all Layout components, as they won't be needed. And set ResizeMode to NONE in inspector. " + "This will boost scrolling performance.",
                adapter.AsMonoBehaviour.gameObject
            );
        }

        private RectTransform InstantiateFromRes(string respath, RectTransform inParent)
        {
            var pref = Resources.Load<GameObject>(respath);
            var inst = (Object.Instantiate(pref, inParent, false) as GameObject).transform as RectTransform;
        ...
        private void DisableLayoutComponentsInTupleValuePrefab(RectTransform tupleValuePrefab)
        {
            var layoutGroup                                             = tupleValuePrefab.GetComponent<LayoutGroup>();
            if (layoutGroup && layoutGroup.enabled) layoutGroup.enabled = false;
