...
namespace Com.ForbiddenByte.OSA.Core
{
    ...
    public abstract class AbstractViewsHolder
    {
        ...
        {
            this.Init(rootPrefab.gameObject, parent, itemIndex, activateRootGameObject, callCollectViews);
        }
        ...
        {
            var go = Object.Instantiate(rootPrefabGO, parent, worldPositionStays);

            this.root = go.transform as RectTransform;
            this.OnRootCreated(itemIndex, activateRootGameObject, callCollectViews);
        }
        ...
        {
            if (existingRoot.parent != parent) existingRoot.SetParent(parent, false);
        ...
        {
            if (this.root) LayoutRebuilder.MarkLayoutForRebuild(this.root);
        }
        ...
        {
            this.ItemIndex = this.ShiftIntWithOverflowCheck(this.ItemIndex, shift, modulo);
        }
        ...
        {
            if (activateRootGameObject) this.root.gameObject.SetActive(true);
            this.ItemIndex = itemIndex;

            if (callCollectViews) this.CollectViews();
        }
