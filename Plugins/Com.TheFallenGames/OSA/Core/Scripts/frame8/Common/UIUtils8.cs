...
namespace frame8.Logic.Misc.Other
{
    ...
    {
        public static UIUtils8 Instance { get; } = new();
        ...
        {
            var     camIfNonOverlay = canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : camera;
            var     screenPoint     = RectTransformUtility.WorldToScreenPoint(camIfNonOverlay, position);
            Vector2 result;
            if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRect, screenPoint, camIfNonOverlay, out result)) return null;
        ...
        public Vector3? ScreenPointToWorldPointInCanvas(Canvas canvas, RectTransform canvasRect, Camera camera, Vector2 screenPoint)
        {
            var     camIfNonOverlay = canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : camera;
            Vector3 result;
            if (!RectTransformUtility.ScreenPointToWorldPointInRectangle(canvasRect, screenPoint, camIfNonOverlay, out result)) return null;
        ...
        {
            foreach (var tr in transforms) this.BringRectToAnchorsFor(tr);
        }

        public void BringRectToAnchorsFor(Transform transform)
        {
            var rt                       = transform as RectTransform;
            if (rt != null) rt.offsetMin = rt.offsetMax = Vector2.zero;
        }

        public void BringAnchorsToRectFor(Transform[] transforms)
        {
            foreach (var tr in transforms) this.BringAnchorsToRectFor(tr);
        }
        ...
        public void BringAnchorsToRectFor(Transform transform)
        {
            ...
            if (transform != null)
            {
                ...
                else
                {
                    ...
                    var dispFromParentBottomLeftCorner =
        ...
        public void GrowSizeDirectionally(RectTransform rectTransform, RectTransform.Edge edge, float amount)
        {
            ...
            var edgeRight  = edge == RectTransform.Edge.Right;
            var horizontal = edgeRight || edge == RectTransform.Edge.Left;
            var edgeTop    = edge == RectTransform.Edge.Top;
            rectTransform.SetSizeWithCurrentAnchors(horizontal ? RectTransform.Axis.Horizontal : RectTransform.Axis.Vertical, horizontal ? currentSize.x + amount : currentSize.y + amount);

            var localPosDiplacementToAdd = Vector3.zero;
            if (horizontal)
            {
                if (edgeRight)
        ...
        public UIVertex[] GetCharQuadVertsInObjectSpaceUnscaledByCanvas(Text textComp, int charIndex)
        {
            ...
            if (charIndex < text.Length)
            {
                ...
                var vertCountPerQuad           = 4;
                var indexOfTextQuadFirstVertex = charIndex * vertCountPerQuad;
        ...
        {
            var vertices = this.GetCharQuadVertsInObjectSpaceUnscaledByCanvas(textComp, charIndex);
            if (vertices == null)
            {
                positions = null;
                return;
            ...
            for (var i = 0; i < positions.Length; i++) positions[i] = vertices[i].position / cScaleFactor;
        }

        public Vector3[] GetCharQuadVertsPositionsInObjectSpaceScaledByCanvas(Text textComp, int charIndex)
        {
            Vector3[] positions;
            this.GetUITextCharQuadVertsPositionsInObjectSpaceScaledByCanvas(textComp, charIndex, out positions);
        ...
        {
            var positions = this.GetCharQuadVertsPositionsInObjectSpaceScaledByCanvas(textComp, charIndex);
            if (positions != null)
            {
                var i  = 0;
        ...
        public Vector3? GetCharQuadCenterPositionInObjectSpaceScaledByCanvas(Text textComp, int charIndex)
        {
            Vector3? res               = null;
            var      positions         = this.GetCharQuadVertsPositionsInObjectSpaceScaledByCanvas(textComp, charIndex);
            if (positions != null) res = (positions[0] + positions[1] + positions[2] + positions[3]) / 4f;
        ...
        {
            Vector3? res               = null;
            var      positions         = this.GetCharQuadVertsPositionsInWorldSpaceScaledByCanvas(textComp, charIndex);
            if (positions != null) res = (positions[0] + positions[1] + positions[2] + positions[3]) / 4f;
        ...
        public Vector3? GetCharQuadCenterLeftPositionInWorldSpaceScaledByCanvas(Text textComp, int charIndex)
        {
            Vector3? res               = null;
            var      positions         = this.GetCharQuadVertsPositionsInWorldSpaceScaledByCanvas(textComp, charIndex);
            if (positions != null) res = (positions[0] + positions[3]) / 2f;
