...
namespace Com.ForbiddenByte.OSA.Editor
{
    internal static class MenuItems
    {
        ...
        private static void OptimizeSelectedScrollRectWithOSA(MenuCommand command)
        {
        ...
        private static void CreateOSA(MenuCommand menuCommand)
        {
            if (!CheckForCreateOSAViaWizard()) return;

            CreateOSAWindow.Open(new());
        }

        [MenuItem("GameObject/UI/Optimized TableView (OSA)", false, 11)]
        private static void CreateTableViewOSA(MenuCommand menuCommand)
        {
            ...
            var p = InitOSAWindow.Parameters.Create(
                validationResult,
                true,
                false,
                false,
        ...
        private static bool CheckForCreateOSAViaWizard()
        {
