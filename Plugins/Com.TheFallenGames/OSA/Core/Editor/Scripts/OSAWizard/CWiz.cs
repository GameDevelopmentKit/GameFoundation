...
namespace Com.ForbiddenByte.OSA.Editor.OSAWizard
{
    public static class CWiz
    {
        ...
        public static string GetExampleItemPrefabResPath(string templateName)
        {
            ...
            var prefabName = templateName + "Item";
        ...
        public static EditorWindow GetBestEditorWindowToShowNotification(bool allowFocusedWindow = true)
        {
            ...
            if (!editorWindow)
            {
                ...
                if (!editorWindow) editorWindow = EditorWindow.GetWindow<SceneView>();
            }
        ...
        {
            if (!editorWindow) editorWindow = GetBestEditorWindowToShowNotification(allowFocusedWindow);

            if (!editorWindow) return;

            try
            {
                editorWindow.ShowNotification(new(msg));
            }
            catch
            {
            }
        ...
        public static void ShowCouldNotExecuteCommandNotification(EditorWindow editorWindow)
        {
            ShowNotification("Invalid state. Check console for details", true, editorWindow);
        }
        ...
        public static bool IsSubclassOfRawGeneric(Type baseType, Type derivedType)
        {
            while (derivedType != null && derivedType != typeof(object))
            {
                ...
                if (baseType == currentType) return true;
        ...
        public static bool IsSubclassOfOSA(Type derivedType)
        {
            return IsSubclassOfRawGeneric(typeof(OSA<,>), derivedType);
        }
        ...
        public static class TV
        {
            ...
            #else
            public static TemplatePaths Paths = new(DIR_RESPATH, false)
            {
            };
            #endif
            ...
            public class TemplatePaths
            {
                ...
                public TemplatePaths(string basePath, bool isTMPro)
                {
                    ...
                    var itemPrefabsSubPathOriginal = "ItemPrefabs";
                    var itemPrefabsSubPath         = itemPrefabsSubPathOriginal;
                    var inputPrefabsSubPath        = "Input";
                    var prefabsSuffix              = "";
                    ...
                    this.SCROLL_VIEW                  = this.BASE_PATH + "/" + TABLE_VIEW_NAME;
                    this.IMPLEMENTATION_TEMPLATE_FILE = TEMPLATE_SCRIPTS_RESPATH + "/" + IMPLEMENTATION_TEMPLATE_NAME;

                    this.TUPLE_PREFAB       = this.BASE_PATH + "/" + itemPrefabsSubPath + "/" + TUPLE_PREFAB_SIMPLE_NAME + prefabsSuffix;
                    this.TUPLE_VALUE_PREFAB = this.BASE_PATH + "/" + itemPrefabsSubPath + "/" + TUPLE_VALUE_PREFAB_SIMPLE_NAME + prefabsSuffix;
                    // COLUMNS_PREFAB is the same for tmpro and non-tmpro setups
                    this.COLUMNS_TUPLE_PREFAB       = this.BASE_PATH + "/" + itemPrefabsSubPathOriginal + "/" + COLUMNS_TUPLE_PREFAB_SIMPLE_NAME;
                    this.COLUMNS_TUPLE_VALUE_PREFAB = this.BASE_PATH + "/" + itemPrefabsSubPath + "/" + COLUMNS_TUPLE_VALUE_PREFAB_SIMPLE_NAME + prefabsSuffix;

                    this.INPUT__FLOATING_DROPDOWN = this.BASE_PATH + "/" + inputPrefabsSubPath + "/" + FLOATING_DROPDOWN_SIMPLE_NAME + prefabsSuffix;
                    this.INPUT__FLOATING_TEXT     = this.BASE_PATH + "/" + inputPrefabsSubPath + "/" + FLOATING_TEXT_INPUT_CONTROLLER_SIMPLE_NAME + prefabsSuffix;
                }
            }
        }
        ...
        public static void GetAvailableOSAImplementations(List<Type> list, bool excludeExampleImplementations, Type implementedInterface = null, Type withoutImplementedInterface = null)
        {
            ...
            foreach (var type in assembly.GetTypes())
            {
                ...
                if (!IsSubclassOfOSA(type)) continue;
