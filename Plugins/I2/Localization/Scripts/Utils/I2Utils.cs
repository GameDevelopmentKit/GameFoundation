namespace I2.Loc
{
    ...
    public static class I2Utils
    {
        ...
        public static string ReverseText(string source)
        {
            ...
            char[] separators = { '\r', '\n' };
            for (var istart = 0; istart < len;)
            {
                var iend           = source.IndexOfAny(separators, istart);
                if (iend < 0) iend = len;
                Reverse(istart, iend - 1);

                for (istart = iend; istart < len && (source[istart] == '\r' || source[istart] == '\n'); istart++) output[istart] = source[istart];
            }

            void Reverse(int start, int end)
            {
                for (var i = 0; i <= end - start; i++) output[end - i] = source[start + i];
            }
        ...
        {
            var  Ind1       = sPath.IndexOf("\\Resources\\", StringComparison.Ordinal);
            var  Ind2       = sPath.IndexOf("\\Resources/", StringComparison.Ordinal);
            var  Ind3       = sPath.IndexOf("/Resources\\", StringComparison.Ordinal);
            var  Ind4       = sPath.IndexOf("/Resources/", StringComparison.Ordinal);
